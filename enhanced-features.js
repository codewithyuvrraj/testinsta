...
function initEnhancedFeatures() {
  ...
  // Handle escape key for post modal
...
window.viewPost = async function(postId) {
  try {
    ...
    const isVideo = post.media_type === 'video' || post.image_url?.includes('video');
    ...
    modal.innerHTML = `
      ...
      <div class="modal-dialog post-modal-dialog">
        <div class="modal-header">
          ...
          <button onclick="closePostModal()" class="close-btn">×</button>
        </div>
        <div class="modal-body post-modal-body">
          <div class="post-media">
            ${post.image_url ? 
              (isVideo ? 
                `<video src="${post.image_url}" controls style="width: 100%; max-height: 400px; object-fit: contain;" />` :
                `<img src="${post.image_url}" alt="Post" style="width: 100%; max-height: 400px; object-fit: contain;" />`
              ) : ''
            }
          </div>
          ${post.caption ? `
            <div class="post-caption">
              <strong>${user?.username || 'User'}</strong> ${post.caption}
            </div>
          ` : ''}
          <div class="post-actions">
            <button class="post-action" onclick="toggleLike('${post.id}')" title="Like">
              ❤️
            </button>
            <button class="post-action" onclick="showPostComments('${post.id}')" title="Comment">
              💬
            </button>
              ...
              ${comments?.map(comment => `
                <div class="comment-item">
                  ...
                  <div class="comment-content">
                    ...
                    <div class="comment-time">${window.formatTime ? window.formatTime(comment.created_at) : new Date(comment.created_at).toLocaleDateString()}</div>
                  </div>
                </div>
        ...
        <div class="post-modal-footer">
          <form class="comment-form" onsubmit="addPostComment(event, '${post.id}')">
...
// Show comments in post modal
window.showPostComments = function(postId) {
...
window.addPostComment = async function(event, postId) {
  ...
  try {
    ...
    // Refresh comments in post modal
    const { data: updatedComments } = await window.sb
      .from('comments')
      .select(`
        *,
        profiles:user_id (username, avatar_url)
    ...
    const commentsList = document.getElementById('postCommentsList');
    if (commentsList && updatedComments) {
...
// Handle backdrop clicks for post modal
document.addEventListener('click', (e) => {
  if (e.target.classList.contains('modal-backdrop')) {
    const modal = e.target.closest('.modal');
    if (modal && modal.id === 'postModal') {
      closePostModal();
    }
  }
});