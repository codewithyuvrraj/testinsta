...
window.addComment = async function(event, postId) {
  ...
  try {
    ...
    // Refresh comments without closing modal
    ...
    if (e.target.classList.contains('modal-backdrop')) {
      ...
      if (modal) {
        if (modal.id === 'commentsModal') {
          ...
          // Restore body scrolling for other modals too
          document.body.style.overflow = '';
        }
      }
    }
...
// Apply to all modal opening functions
const originalOpenCreatePostModal = window.openCreatePostModal;
if (originalOpenCreatePostModal) {
  window.openCreatePostModal = function() {
    preventBodyScroll();
    return originalOpenCreatePostModal.apply(this, arguments);
  };
}

const originalOpenCreateStoryModal = window.openCreateStoryModal;
if (originalOpenCreateStoryModal) {
  window.openCreateStoryModal = function() {
    preventBodyScroll();
    return originalOpenCreateStoryModal.apply(this, arguments);
  };
}

const originalOpenCreateReelModal = window.openCreateReelModal;
if (originalOpenCreateReelModal) {
  window.openCreateReelModal = function() {
    preventBodyScroll();
    return originalOpenCreateReelModal.apply(this, arguments);
  };
}
